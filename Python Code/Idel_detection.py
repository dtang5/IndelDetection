...
def num_lines_in_file(filename):
    ...
    filename = name of the file containing the samples, example: filename.txt

    how to call: num_of_lines = num_lines_in_file('filename.txt')

    ***This is more of a nice to know method. This function is never used in the quantification of DNA.***
    '''
...
def generate_sequence_file(filename, filewritename):
    '''
    Takes individual sequences from the original file of samples and writes only the line containing the sequences into a new file of user-inputted name

    filename =  name of the file containing the samples, example: filename.txt

    filewritename = new name of the file in which the sequence-only file will be stored. This is user-inputted, example: newfilename.txt (you must types .txt at the end of your filename)
    '''
    seq_file = open(filewritename, 'w')
    with open(filename, 'r') as f:
        for count, line in enumerate(f, start=1):
            if (count + 2) % 4 == 0:  # DNA sequence appears once every 4 lines (2nd one in the list)
                seq_file.write(line)


def generate_quality_file(filename, filewritename):
    '''
    Takes individual sequence qualities from the original file of samples and writes only the line containing the sequence qualities into a new file of user-inputted name

    filename =  name of the file containing the samples, example: filename.txtw
...
def quality_quantification(filename, filewritename, num_values_quantized):
    '''
    Quantifies the quality of each individual sequence according to fastq documentation

    filename =  name of the file containing the samples, example: filename.txt

    filewritename = new name of the file in which the sequence-quantified file will be stored. This is user-inputted, example: newfilename.txt (you must types .txt at the end of your filename)

    num_values_quantized = number of quality values you want to take in account per line of sequence quality values. Rationale: quality gets worse as the sequence progresses, look at sequence file to coordinate this
    '''

    quality_file = list(open(filename, 'r'))  # converted to list for efficiency in future computations

    # Implementation of a hash table (dictionary) for an O(constant) implementation -- saves a lot of time since the quantification of sequence qualities is repetitive
    fastq_quality = {}
    qualities = '\n!"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~'  # From fastq documentation arranged from lowest quality to highest quality; the \n is added becauses it's technically a newline character and will get no value (0)
    #
    ...
    quality_quant = open(filewritename,
                         'w')  # creates the file that will receive the written quality number totals per sequence

    for i in range(len(quality_file)):  # iterates through each element of the quality_file list
        qual_sum = 0
        for j in range(1, num_values_quantized + 2):  # ignore first character of each line
            qual = fastq_quality[
                quality_file[i][j]]  # dictionary value of char j in the quality line i of quality_file
...
def remove_bad_quality_seq(sequence_only_file, quality_quantified_file, filewritename):
    ...
    quality_quant_file = [int(i) for i in
                          quality_quant_file]  # quality_quant_file = list(map(int,list(open(quality_quantified_file,'r')))) also works but a little slower
    ...
    for index in range(len(quality_quant_file)):
        if quality_quant_file[index] < (median_quality - (0.25 * std_quality)):
            pass
        else:
...
def percent_indels(filename):
    ...
    # dictionary
    mouse_barcodes = {"TCACG": 0, "TAGGC": 1, "CAGTG": 2, "AGATC": 3, "ATCAG": 4, "GCTAC": 5, "GTCAA": 6, "TGTCA": 7,
                      "TAGAG": 8, "TGAAA": 9, "TTTCG": 10, "AGTGG": 11, "CTGAT": 12, "TTCCT": 13, "AACTA": 14}
    for sequence in sequence_list:
        for barcode in mouse_barcodes:
            i = mouse_barcodes.get(barcode, 0)
            if (sequence[1:6] == barcode) and (
                    sequence[start_basenum_of_qual_ens_seq:end_basenum_of_qual_ens_seq] == quality_ensuring_sequence):
    ...
    mouse_names = ['RP1', 'RP10', 'RP11', 'RPNM', 'RQ1', 'RQ8', 'RQ10', 'RQ11', 'RQNM', 'SR2', 'SR10', 'SR11', 'SY1',
                   'SY2', 'SY10']
    indel_percents = {}  # dict
    # frame_shift_percents = {}

    for index in range(len(mouse_names)):
        percent_indels = (mut[index] / (mut[index] + reg[index])) * 100
        percent_frame_shifts_if_mutants = ((frame_shift[index] / mut[index]) * 100)
...
def main():
    ...
    remove_bad_quality_seq('RPRQ_1_seq_only.txt', 'RPRQ_1_quality_quantification_first_86.txt',
                           'RPRQ_1_seq_only_modified_for_quality_first_86.txt')
